...
L-67\_A         & N boundary  & Water Level     & BC\_S155+\_HW.dfs0 \\
...
Canal L-29 is hydraulically disconnected at chainage 18430 (m) using a Closed Discharge structure named TT-CLOSED. This structure separates the western section of L-29 which is hydraulically connected to canal L-28 and maintains stages lower than the stages within WCA-3A. The central sec section of L-29 (between L-29 and L-67C) are hydraulically connected to WCA-3A through overland links.

%----------------------------------------------------------
\subsubsection{S343A}
%----------------------------------------------------------

This structure is implemented in MIKE 11 as a Discharge structure with a Max Rate of change of 1 $cfs/s$. the structure discharges into the western section of Canal L29.
...
\item[]
\item Priority 5: If $H<0$, operate according to Table~\ref{tab:CS-S333close}.
\end{packed_items}
...
\item[]
\item Priority 5: If $H<0$, operate according to Table~\ref{tab:CS-S334close}.
\end{packed_items}
...
The S-173 structure closes if the difference in water level between upstream and downstream is negative to prevent having S-331S discharge recirculate back upstream. The structure is modeled as Underflow Gate with width of 6 ft, and max rate of gate opening of 0.008333333 ft/sec, which equals 6 inch/hour.
...
\begin{packed_items}
\item Priority 1: If $S176HW > 5.5$, and $Mode>=0$, and  S331P flow $<$ 0.1 cfs, and $S173HW > S173TW$ operate according to Table~\ref{tab:CS-S173ops-open}. %all modes
...
\begin{packed_items}
\item Priority 1: If S331P flow $<=$ 0.0 cfs, and S176HW $<$ 5.5 ft, and $Mode>=0$  discharge according to control strategy in Table~\ref{tab:CS-S331SA}.
\item Priority 2: Close when headwater is less than 0.0 ft.
\end{packed_items}
...
\begin{packed_items}
\item Priority 1: If $Mode>=0$ and tailwater at S-331 $>=$ 6.00 ft., close.
\item Priority 2: If $Mode>=0$ and headwater at S-176 $>$ 5.5 ft., close.
\item Priority 3: If $Mode=0$ and headwater S-331 $>$ 0.0, discharge according to Table~\ref{tab:CS-S331Pa}
\item Priority 4: If $Mode>=0$ and water level at Angels Well is $>=$ 6.00 ft., discharge according to Table~\ref{tab:CS-S331Pa}
\item Priority 5: If $Mode>=0$ and water level at Angels Well is $>=$ 5.75 ft., discharge according to Table~\ref{tab:CS-S331Pb}
\item Priority 6: If $Mode>=0$ and water level at Angels Well is $>=$ 5.50 ft., discharge according to Table~\ref{tab:CS-S331Pc}
\item Priority 7: Close when H $<$ 0.0
\end{packed_items}
...
\item[]
\item Priority 6: Default, operate according to Table~\ref{tab:CS-S338close}.
\end{packed_items}
...
\item[]
\item Priority 6: Default, operate according to Table~\ref{tab:CS-S194close}.
\end{packed_items}
...
\item Priority 5: If $Mode=2$ and $H_{us}>=4.5 ft$, remain unchanged.
\item[]
\item Priority 6: Default, operate according to Table~\ref{tab:CS-S196close}.
\end{packed_items}
...
\item Priority 3: If $Mode=0$ and $H_{us}<4.75 ft$, Close.
\item[]
...
\begin{packed_items}
\item Priority 1: When $H_{us} > 3.8$ ft and $Mode>0$, discharge according to Table~\ref{tab:CS-S200}
\item Priority 2: When $H_{us} > 3.6$ ft and $Mode>0$, remain unchanged
\item Priority 3: When $Mode=0$, $Q = Q_{obs}$
...
\item Priority 3: If $Mode=0$ and $H_{us}<3.6 ft$, Close.
\item[]
...
\begin{packed_items}
\item Priority 1: When HW $>$ 3.8 ft and $Mode>0$, discharge according to Table~\ref{tab:CS-S199}
\item Priority 2: When HW $>$ 3.6 ft and $Mode>0$, remain unchanged
\item Priority 3: When $Mode=0$, $Q = Q_{obs}$
\item Priority 4: When stage $<$ 0.0, close
\end{packed_items}
...
\item[]
\item Priority 2: If $Mode=0$ and $H_{us}>=2.6 ft$, operate according to Table~\ref{tab:CS-S18Copen}.
\item Priority 3: If $Mode=0$ and $H_{us}<2.3 ft$, Close.
\item[]
...
For the calibration period the structure uses observed discharges (Structure S197-Q)
...
\item Priority 3: If S-176 HW $<$ 5.2 ft, $Mode>0$, and if S-177 HW $<$ 4.2 ft., close.
...
\item Priority 3: If S-176 HW $<$ 5.2 ft and if S-177 HW $<$ 4.2 ft and $Mode>0$, close.
\item Priority 4: Default, close.
\end{packed_items}
