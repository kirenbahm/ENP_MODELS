...
\begin{packed_items}
\item Priority 1: If $Mode>=0$, $Q = Q_{obs}$
\item Priority 2: If $Mode>=1$, $ANGELS\_WELL >=  6.0$, Discharge according to Table~\ref{tab:CS-S331Pa1}
\item Priority 3: If $Mode>=1$, $ANGELS\_WELL >=  5.75$, No Change.
\item Priority 4: If $Mode>=1$, $ANGELS\_WELL > 5.5$, discharge according to Table~\ref{tab:CS-S331Pa2}
\item Priority 5: If $Mode>=1$, $ANGELS\_WELL <  5.5$ ft., close.
\item Priority 6: Close.
\end{packed_items}

% this table will lists pump schedule for S331HW > 4.0, it  we also kneed to more about how many pumps are open.
\footnotesize
\begin{table}[!h]
\centering
\caption{Control strategy for Priority 2 at S331P}
\label{tab:CS-S331Pa1}
\begin{tabular}{p{1.2in}{r} p{0.5in}{r}}
\hline
...
0	&	0     \\
4.0	&   0     \\
4.1	&	1160  \\    % here a scaling can be done depending on the water levels,
                    % i.e. 1 Pump, 2 Pumps, 3 Pumps based on the headwater, but we don't have the information
4.5	&	1160  \\
10	&	1160  \\
\hline
\end{tabular}
...
% this table will keep pump operating for S331HW > 4.5
\footnotesize
\begin{table}[!h]
\centering
\caption{Control strategy for Priority 4 at S331P}
\label{tab:CS-S331Pa2}
\begin{tabular}{p{1.2in}{r} p{0.5in}{r}}
\hline
\textbf{HW (ft)} & \textbf{Q (cfs)}\\
\hline
0	&	0         \\
4.5	&   0        \\
4.6	&	1160     \\     % here a scaling can be done depending on the water levels,
                        % i.e. 1 Pump, 2 Pumps, 3 Pumps based on the headwater, but we don't have the information
5.0	&	1160     \\ %
10	&	1160     \\
\hline
...
\begin{packed_items}
\item Priority 1: When $Mode=0$, $Q = Q_{obs}$
\item Priority 2: When $Mode=1$, $H_{us} > 3.8$ ft, YEAR >= 2012 and MONTH >=7 discharge according to Table~\ref{tab:CS-S200}
\item Priority 3: When $Mode=1$, $H_{us} > 3.8$ ft, YEAR >= 2013 discharge according to Table~\ref{tab:CS-S200}
\item Priority 4: When $Mode=1$, $H_{us} > 3.6$ ft, remain unchanged
\item Priority 5: Close.
\end{packed_items}
...
\begin{packed_items}
\item Priority 1: When $Mode=0$, $Q = Q_{obs}$
\item Priority 2: When $Mode=1$, $H_{us} > 3.8$ ft, and YEAR >= 2012 and MONTH >=6 discharge according to Table~\ref{tab:CS-S199}
\item Priority 3: When $Mode=1$, $H_{us} > 3.8$ ft, YEAR >= 2013 discharge according to Table~\ref{tab:CS-S199}
\item Priority 4: When $Mode=1$, $H_{us} > 3.6$ ft, remain unchanged
\item Priority 5: Close.
\end{packed_items}
